...
export default function IgvOracao() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* Header */}
      <header className="bg-white border-b border-black/[0.06] px-4 pt-safe-top">
        <div className="flex items-center gap-3 h-14">
          <Link to="/igv" className="p-1.5 -ml-1.5 rounded-lg text-gray-500 hover:bg-gray-100 transition-colors">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-gray-900 text-[0.9rem]">Pedidos de Oração</span>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 px-4 py-6 max-w-lg mx-auto w-full">
        ...
        {state === 'success' ? (
          ...
          <div className="flex flex-col items-center text-center py-12 gap-5">
            ...
            <div>
              <h2 className="text-[1.1rem] font-bold text-gray-900 mb-1.5">Pedido recebido!</h2>
              <p className="text-gray-500 text-[0.85rem] leading-relaxed max-w-xs mx-auto">
                Estamos orando por você. Nossa equipe pastoral receberá seu pedido e levará ao Senhor.
              </p>
            </div>
            <Link
              to="/igv"
              className="mt-2 px-6 py-2.5 rounded-xl font-semibold text-[0.85rem] text-white transition-opacity active:opacity-80"
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="mb-1">
              <h1 className="text-[1.15rem] font-bold text-gray-900">Compartilhe seu pedido</h1>
              <p className="text-gray-400 text-[0.8rem] mt-1 leading-relaxed">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[0.8rem] font-semibold text-gray-600">Seu nome</label>
              <input
                type="text"
                value={name}
                ...
                className="w-full px-4 py-3 rounded-xl border border-black/[0.1] bg-white text-[0.9rem] text-gray-900 placeholder:text-gray-300 focus:outline-none focus:ring-2 transition-shadow"
            ...
            {/* Telefone */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[0.8rem] font-semibold text-gray-600">WhatsApp</label>
              <input
                type="tel"
                value={phone}
            ...
            {/* Pedido */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[0.8rem] font-semibold text-gray-600">Seu pedido de oração</label>
              <textarea
                value={text}
                ...
                minLength={10}
                rows={5}
                className="w-full px-4 py-3 rounded-xl border border-black/[0.1] bg-white text-[0.9rem] text-gray-900 placeholder:text-gray-300 focus:outline-none focus:ring-2 transition-shadow resize-none"
                style={{ '--tw-ring-color': `${IGV.primaryColor}40` } as React.CSSProperties}
              />
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl font-semibold text-[0.9rem] text-white transition-opacity active:opacity-80 disabled:opacity-60 mt-1"
              style={{ backgroundColor: IGV.primaryColor }}
            >
              {state === 'submitting' ? (
