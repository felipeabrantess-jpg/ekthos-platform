/**
 * AcolhimentoDashboard — /agentes/agent-acolhimento/dashboard
 *
 * Métricas em tempo real do agente de acolhimento:
 * 6 cards + empty states por situação.
 *
 * Empty states:
 *  - subscription_status ausente/null → sem contrato → CTA "Contratar Acolhimento"
 *  - subscription_status = 'pending_activation' → aguardando ativação (até 1h)
 *  - journeys_ativas = 0 e creditos_consumidos_mes = 0 → aguardando primeira visita
 */
...
interface DashboardData {
  journeys_ativas: number
  journeys_por_status: Record<string, number>
  mensagens_semana: number
  conversas_handoff: number
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
      <div className="bg-white rounded-3xl border border-black/[0.06] shadow-sm p-10 max-w-md w-full text-center">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-[#FDE8E0] mb-6">
          <Zap size={28} className="text-[#e13500]" />
        </div>
        ...
        <p className="text-[#5A5A5A] text-sm leading-relaxed mb-6">
          O Agente de Acolhimento não está ativo na sua conta.
          Ele entra em contato automaticamente com cada visitante em até 24h,
          com linguagem pastoral.
        </p>
        <Link
          to="/agentes/agent-acolhimento"
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
      <div className="bg-white rounded-3xl border border-black/[0.06] shadow-sm p-10 max-w-md w-full text-center">
        ...
        <p className="text-[#5A5A5A] text-sm leading-relaxed mb-4">
          Seu pagamento foi confirmado. O agente de acolhimento está sendo configurado
...
function EmptyNoActivity() {
  return (
    <div className="mb-6 rounded-2xl border border-[#EDE0CC] bg-[#FDF6EB] p-5 flex items-start gap-4">
      ...
      <div>
        <p className="font-semibold text-[#161616] text-sm mb-1">Aguardando primeira visita</p>
        <p className="text-[#5A5A5A] text-sm leading-relaxed">
          Quando um visitante registrar o número via QR Code ou ficha, o agente entra em ação automaticamente.
          Configure o QR Code em{' '}
          <Link to="/configuracoes/qr-visitante" className="text-[#e13500] underline underline-offset-2">
            Configurações → QR Visitante
          </Link>
          .
        </p>
      </div>
...
export default function AcolhimentoDashboard() {
  ...
  if (data.subscription_status === 'pending_activation') {
    return (
      <div className="min-h-screen bg-[#f9eedc]">
        <Header onRefresh={() => load(true)} refreshing={refreshing} />
        <div className="max-w-5xl mx-auto px-6">
  ...
  const creditosCanon = 600 // agent-acolhimento
  ...
  const statusLabels: Record<string, string> = {
    pending:      'Aguardando 1ª mensagem',
    contacted:    'Primeiro contato feito',
    responded:    'Pastor respondeu',
    follow_up_1:  'Follow-up 1 enviado',
    follow_up_2:  'Follow-up 2 enviado',
    converted:    'Convertido (membro)',
    lost:         'Perdido',
    unsubscribed: 'Optou por sair',
  }

  const journeysEntries = Object.entries(data.journeys_por_status ?? {})
  ...
  return (
    <div className="min-h-screen bg-[#f9eedc]">
      ...
      <main className="max-w-5xl mx-auto px-6 pb-12">
        ...
        {/* Empty: sem atividade */}
        {data.journeys_ativas === 0 && data.creditos_consumidos_mes === 0 && (
          <EmptyNoActivity />
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          <MetricCard
            ...
            value={data.journeys_ativas}
            sub="visitantes em acompanhamento"
          />
          <MetricCard
            icon={MessageSquare}
            iconBg="bg-[#E3F2FD]"
            ...
            label="Mensagens / 7d"
            value={data.mensagens_semana}
            sub="créditos consumidos na semana"
          />
          <MetricCard
            icon={Headphones}
            iconBg="bg-[#F5E0E0]"
            iconColor="text-[#670000]"
            label="Aguardando pastor"
            value={data.conversas_handoff}
            sub="conversas abertas no handoff"
          />
          <MetricCard
            icon={CreditCard}
            iconBg="bg-[#E8F5E9]"
          ...
          <MetricCard
            ...
            value={fmtDateShort(data.proxima_renovacao)}
            sub="créditos renovam automaticamente"
          />
        </div>

        {/* ── Distribuição por status ──────────────────────────── */}
        {journeysEntries.length > 0 && (
          <div className="bg-white rounded-2xl border border-black/[0.06] shadow-sm p-6 mb-6">
            <div className="flex items-center gap-2 mb-5">
              ...
              <h3 className="font-semibold text-[#161616] text-sm">Distribuição de jornadas</h3>
            </div>
            <div className="space-y-3">
              {journeysEntries.map(([status, count]) => {
                const total = Object.values(data.journeys_por_status).reduce((a, b) => a + b, 0)
                ...
                return (
                  <div key={status}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="font-mono text-sm font-medium text-[#161616]">
                        {count} <span className="text-[#8A8A8A] text-xs">({pct}%)</span>
                      </span>
                    </div>
                    <div className="h-1.5 bg-[#f9eedc] rounded-full overflow-hidden">
                      <div
                        className="h-full bg-[#e13500] rounded-full transition-all duration-500"
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link
            to="/agentes/agent-acolhimento/configurar"
            className="inline-flex items-center gap-2 bg-white rounded-xl border border-black/[0.08]
                       px-4 py-2.5 text-sm font-medium text-[#161616]
                       hover:border-[#e13500] hover:text-[#e13500] transition-colors duration-150"
          >
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-[#5A5A5A] text-sm mt-1">
              Métricas em tempo real — primeiros contatos com visitantes
            </p>
          </div>
          <button
            onClick={onRefresh}
