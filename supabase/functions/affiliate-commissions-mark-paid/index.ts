...
Deno.serve(async (req: Request) => {
  ...
  const { error: auditErr } = await supabase.rpc('record_audit_event', {
    ...
    p_action:                   'affiliate.commissions.mark_paid',
    p_before:                   null,
    p_after:                    { batch_id: body.batch_id, paid_at: paidAt, commissions_count: count },
    p_reason:                   null,
  ...
  if (auditErr) console.error('[affiliate-commissions-mark-paid] audit failed:', auditErr.message)
