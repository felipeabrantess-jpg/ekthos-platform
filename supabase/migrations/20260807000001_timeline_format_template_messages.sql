-- Migration: 20260807000001_timeline_format_template_messages
--
-- Problema: get_person_timeline retornava o JSON raw de templates WhatsApp
-- no campo summary, em vez de texto legível.
-- Causa: conversation_messages.content armazena o payload completo do template
-- Meta (tipo {"type":"template","name":"acolhimento_boasvindas",...}).
-- Solução: helper function format_wa_content + CR na get_person_timeline.

-- ── 1. Helper: format_wa_content ─────────────────────────────────────────────
-- Recebe o texto bruto de content, devolve:
--   - Se for JSON de template Meta: "nome do template — \"param1, param2\" (com vídeo)"
--   - Qualquer outra coisa: LEFT(content, 300) sem alteração

CREATE OR REPLACE FUNCTION public.format_wa_content(p_content text)
RETURNS text
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
  ...
  v_media    text;
BEGIN
  -- Guarda rápido: não começa com '{' → texto normal
  IF p_content IS NULL OR LEFT(p_content, 1) != '{' THEN
    RETURN LEFT(p_content, 300);
  END IF;

  -- Tentar fazer parse; se falhar → retornar como texto
  BEGIN
    v_json := p_content::jsonb;
  EXCEPTION WHEN OTHERS THEN
    RETURN LEFT(p_content, 300);
  END;

  -- Só processa se for um template Meta
  IF (v_json ->> 'type') IS DISTINCT FROM 'template' THEN
    RETURN LEFT(p_content, 300);
  END IF;

  -- Nome do template: underscores → espaços
  v_name := COALESCE(
    REPLACE(v_json ->> 'name', '_', ' '),
    'mensagem'
  );

  -- Parâmetros de texto do componente body
  SELECT string_agg(p.txt, ', ' ORDER BY p.ord)
  INTO   v_texts
  FROM (
    SELECT
      param ->> 'text'  AS txt,
      row_number() OVER () AS ord
    FROM jsonb_array_elements(v_json -> 'components') AS comp
    CROSS JOIN jsonb_array_elements(comp -> 'parameters') AS param
    WHERE (comp ->> 'type') = 'body'
      AND (param ->> 'type') = 'text'
      AND (param ->> 'text') IS NOT NULL
  ) p;

  -- Mídia no header (video, image, document)
  SELECT MAX(param ->> 'type')
  INTO   v_media
  FROM   jsonb_array_elements(v_json -> 'components') AS comp
  CROSS JOIN jsonb_array_elements(comp -> 'parameters') AS param
  WHERE  (comp ->> 'type') = 'header'
    AND  (param ->> 'type') IN ('video', 'image', 'document');

  RETURN CONCAT(
    v_name,
    CASE WHEN v_texts IS NOT NULL THEN ' — "' || v_texts || '"' ELSE '' END,
    CASE WHEN v_media IS NOT NULL THEN ' (com ' || v_media || ')' ELSE '' END
  );
END;
$$;

-- Grants para a helper (public/anon não têm acesso; só authenticated via timeline)
REVOKE ALL ON FUNCTION public.format_wa_content(text) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.format_wa_content(text) TO authenticated;

-- ── 2. Atualizar get_person_timeline ─────────────────────────────────────────
-- Única mudança: LEFT(cm.content, 300) → format_wa_content(cm.content)
...
-- Manter grants originais
REVOKE ALL ON FUNCTION public.get_person_timeline(uuid, integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_person_timeline(uuid, integer) FROM anon;
