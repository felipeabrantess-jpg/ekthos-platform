-- ============================================================
-- Frente 3A — Migration 6 (numerada como 100006)
-- RPC upsert_church_cadastro_cristalino (Etapa 1 do wizard)
-- Atômico: UPDATE churches + soft-delete contractor antigo + INSERT novo contractor
-- Transição de estado: onboarding_step → 'pastoral'
...
BEGIN
  ...
  UPDATE public.churches SET
    ...
    phone                         = COALESCE(NULLIF(TRIM(p_church_data->>'phone'), ''), phone),
    email                         = COALESCE(NULLIF(TRIM(p_church_data->>'email'), ''), email),
...
COMMENT ON FUNCTION public.upsert_church_cadastro_cristalino(uuid, jsonb, jsonb)
  IS 'Etapa 1 do wizard: atualiza churches + soft-delete contractor antigo + INSERT novo. Transição onboarding_step → pastoral (Frente 3A)';
