-- Fase 3: Sincroniza person_pipeline.stage_id e people.pipeline_stage_id
-- dentro da mesma transação de journey_register_attendance.
-- Só stage_id e pipeline_stage_id — nenhuma outra coluna, nenhuma outra tabela.
...
BEGIN
  ...
  --    Só executa quando p_new_stage_id foi passado (intenção explícita de mover).
  --    Upsert garante que linha em person_pipeline existe mesmo para pessoas sem histórico.
  ...
  -- 4. Registra evento de contato pastoral (somente com jornada ativa)
